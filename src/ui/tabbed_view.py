"""
Main application view with tabs - Coordinates UI and business logic.
...
class FileOrganizerApp:
    ...
    Manages organizer view and settings view.
    """
    ...
    def __init__(self, page: ft.Page):
        ...
        # Initialize views
        self.organizer_view = OrganizerTab(page, self.config)
        ...
        tabs = ft.Tabs(
            ...
            tabs=[
                ft.Tab(
                    ...
                    content=self.organizer_view.get_view(),
                ),
                ft.Tab(
                    text="Configuración",
                    icon=ft.Icons.SETTINGS,
                    content=self.settings_view.get_view(),
                ),
            ],
            expand=True,
            indicator_color=Theme.ACCENT,
            label_color=Theme.TEXT,
            unselected_label_color=Theme.SUBTEXT,
        )
        
        self.page.add(tabs)
    
    def _on_config_changed(self) -> None:
        """Called when configuration changes - refresh organizer."""
        self.organizer_view.on_config_changed()


class OrganizerTab:
    """
    File organizer tab - Coordinates UI and services.
    Single Responsibility: UI coordination and user interaction.
    Dependency Inversion: Depends on service abstractions.
    """
    
    def __init__(self, page: ft.Page, config: UserConfig):
        self.page = page
        self.config = config
        self.source_path = config.last_source_path
        self.dest_path = config.last_dest_path
        self.plan: list[FileItem] = []
        self.status_texts: list[ft.Text] = []
        
        # Initialize services with configurable categorizer
        categorizer = ConfigurableFileCategorizer(config)
    ...
    async def pick_source(self, e: ft.ControlEvent) -> None:
        ...
        if path:
            ...
            self.config.last_source_path = path
            self.config.save()
            self.page.update()
    ...
    async def pick_dest(self, e: ft.ControlEvent) -> None:
        ...
        if path:
            ...
            self.config.last_dest_path = path
            self.config.save()
            self.page.update()
    ...
    def run_preview(self, e: ft.ControlEvent) -> None:
        ...
        def scan() -> None:
            ...
            plan = self.org_service.scan_and_plan(
                ...
                Path(self.dest_path),
                skip_hidden=self.config.skip_hidden_files,
            )
    ...
    def display_plan(self, plan: list[FileItem]) -> None:
        ...
        for item in plan:
            ...
            # Show "Sin fecha" if no date folder
            date_display = item.date if item.date else "Sin fecha"
            ...
            self.data_table.rows.append(ft.DataRow(cells=[
                ...
                ft.DataCell(ft.Text(date_display, color=Theme.TEXT, size=11)),
    ...
    def on_config_changed(self) -> None:
        """Called when configuration changes - reinitialize categorizer."""
        categorizer = ConfigurableFileCategorizer(self.config)
        self.org_service = OrganizationService(categorizer)
    
    def get_view(self) -> ft.Column:
        """Get the organizer view control."""
        return self.view
